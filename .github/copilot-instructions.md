...
* **Project:** Property Advisor – A real estate brokerage site for the Sri Lankan (Colombo-focused) market.
...
* **Current State:** Frontend shell is complete using local fixtures. Backend (Render) has DB layer and agent loop, but the API endpoints (`POST /chat`, `GET /properties`) are not yet wired. Wiring these is the immediate next step.
...
**Workflow & Token Efficiency (Copilot Rules)**

* **Direct Implementation:** We are no longer using a spec-driven workflow (`draft-spec`, etc.) to save tokens and time. Implement features directly based on prompts and refer to `context/PROJECT_OVERVIEW.md` for existing data models.
...
* **No Boilerplate:** Omit conversational filler, explanations of what you are about to do, or apologies. Output only the necessary code or direct, concise answers.

**Frontend (Next.js 16 + Tailwind v4)**

* **Stack:** Next.js 16.2.12 (App Router), React 19.2.4, Tailwind v4, TypeScript, `pnpm` (run from `frontend/`).
...
* **Execution:** `uv sync`, `uv run fastapi dev app/main.py`, `uv run alembic upgrade head`, `uv run pytest`.
* **Testing:** Pytest uses an in-memory SQLite database, while production uses Neon Postgres.
* *Gotcha:* Models must stay portable (e.g., use `Uuid`, `func.now()`, `ARRAY(Text).with_variant(JSON(), "sqlite")`).
